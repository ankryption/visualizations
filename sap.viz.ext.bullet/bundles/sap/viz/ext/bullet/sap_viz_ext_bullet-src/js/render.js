/*<<dependency*/
define("sap_viz_ext_bullet-src/js/render", ["sap_viz_ext_bullet-src/js/utils/util"], function(util){
/*dependency>>*/ 
...
*/
    ...
    var render = function(data, container, width, height, colorPalette, properties, dispatch) {
        ...
        var retrievedObject = undefined ;
      
        retrievedObject  = localStorage.getItem('gradientData');
        retrievedObject = JSON.parse(retrievedObject);

        //console.log(' retrievedObject: ', retrievedObject);
        //console.log(' persistedFlag: ', persistedFlag);

        update();
        
        d3.select('.vis').on("DOMNodeRemovedFromDocument", function() {

            console.log("persisted: " + persistedData[0].length);    
            if(data.meta.measures(1)[0] === "actual")
                persistedFlag = true ;

            d3.selectAll("#t-tip").remove();
            ...
            for (var bullet=0 ; bullet<bulletCount ; bullet++){
                ...
                for(var box=0 ; box<colorBoxes ; box++){
                    ...
                    var Obj = { "widthRatio": widthRatio, "color": color };
                    boxData.push(Obj);
                }
        ...
        function customBulletChart(config) {
            ...
            this.drawBullet = function (_dataBullet, counter, bulletArea){
                ...
                var yScale = d3.scale.linear()
                                        ...
                                        .range([0, chartH]);
                    /*
                    d3.select(".legendArea").append("foreignObject")
                                                .attr('y',yScale(counter))
                                                .attr("width", 50)
                                                .attr("height", chartH)
                                                .append("xhtml:body")
                                                .style("font", "14px 'Helvetica Neue'")
                                                .html(_dataBullet.title);
                    */
                if(flagDimension){
                ...
                if(persistedFlag === false){
                    ...
                    for ( ; boxCount<numColor ; boxCount++){
                        ...
                        box[boxCount].append("rect")
                                            ...
                                            .on("click",function(){
                                                                var temp =d3.selectAll("#t-tip").style("display","none");
                                                                d3.select('.colorPicker').style("display","none");
                                                                var rect = this;
                        ...
                        for (var boxCount=0 ; boxCount<numColor ; boxCount++){
                            box[boxCount] = gradientBoxes.append("g").attr("class","box" + boxCount);                                
                            box[boxCount].append("rect")
                                                .attr("x",0)
                                                .attr("y",yScale(counter))
            ...
            // plannedBar            
                if(flag){
                    ...
                    bullet.append("g").attr("class", "xAxis")
                                        .attr("transform", "translate(" + 0 + "," + (yScale(counter) + objHeight) + ")")
                                        .call(xAxis);

                    
                var resizer = d3.selectAll('#resizer');
            ...
            this.onMouse = function(_dataBullet, _selection, counter, this1, tooltipOffset) {
                ...
                if (this1.getAttribute("class") === "actualBar" || this1.getAttribute("class") === "plannedBar"){
                ...
                var arrowH = 3;
                    ...
                    //leftPos = (window.pageXOffset + matrix.e + xScale(_dataBullet.total/2) - objHeight);
                ...
                var tt_data;
                if(flagDimension)
                    tt_data="<div class='tt-item tt-combo'>"+ (_dataBullet.title)+"</div>";
                
                else
                    tt_data="<div class='tt-item tt-combo'>"+ " " +"</div>";

                    tt_data=tt_data+"<div class='tt-item tt-bag-val'> actual&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
                    tt_data=tt_data+"<span class='tt-item-price'>"+_dataBullet.actual+"</span></div>";
                    if(flag){
                        tt_data=tt_data+"<div class='tt-item tt-bag-val'> Target";
                        tt_data=tt_data+"<span class='tt-item-price'>"+_dataBullet.planned+"</span></div>";
                        topPos = topPos - 20;
                    }
                    if(!flagTotal){
                        tt_data=tt_data+"<div class='tt-item tt-bag-val'> Total";
                        tt_data=tt_data+"<span class='tt-item-price'>"+_dataBullet.total+"</span></div>";
                        topPos = topPos - 20;    
                    }
            ...
            this.outMouse = function(){
                ...
                if (this.getAttribute("class") === "actualBar" || this.getAttribute("class") === "plannedBar"){
                    event.currentTarget.parentNode.childNodes[2].setAttribute("filter", "");
                    //event.currentTarget.parentNode.childNodes[3].setAttribute("filter", "");
                }
                else{
                    event.currentTarget.parentNode.parentNode.parentNode.childNodes[2].setAttribute("filter", "");
                    //event.currentTarget.parentNode.parentNode.parentNode.childNodes[3].setAttribute("filter", "");
                }
            ...
            this.drag = function(){
                x = d3.mouse(this.parentNode.childNodes[0])[0];
                if(this.getAttribute('x')!==0){
                    resizerId = parseInt(this.getAttribute("class"));
                    ...
                    //this.parentNode.childNodes[0].setAttribute('width', x + 'px');
    ...
    */
        function update() {
            ...
            if(flagDimension){
                if(flag && !flagTotal){
                    var fdata = data.map(function(d) {
                        dataset.push({"title":d[ds0], "total":d[ms0], "actual":d[ms1], "planned":d[ms2]});
                        });
                }
                else{
                    if(flagTotal){
                        var fdata = data.map(function(d) {
                            ...
                            if(d[ms0] > d[ms1])
                                ...
                                temp = d[ms1];
                            dataset.push({"title":d[ds0], "total":temp, "actual":d[ms0], "planned":d[ms1]});
                            });
                    }
                    else{
                        var fdata = data.map(function(d) {
                            dataset.push({"title":d[ds0], "total":d[ms0], "actual":d[ms1]});
                            });
                    }
                } 
            }
                ...
                if(flag && !flagTotal){
                    var fdata = data.map(function(d) {
                        dataset.push({"total":d[ms0], "actual":d[ms1], "planned":d[ms2]});
                        });
                }
                else{
                    if(flagTotal){
                        var fdata = data.map(function(d) {
                            ...
                            if(d[ms0] > d[ms1])
                                ...
                                temp = d[ms1];
                            dataset.push({"total":temp, "actual":d[ms0], "planned":d[ms1]});
                            });
                    }
                    else{
                        var fdata = data.map(function(d) {
                            dataset.push({"total":d[ms0], "actual":d[ms1]});
                            });
                    }
                } 
            }   
            ...
            var legendArea = vis.append("svg").attr("width", legendPad).attr("height", chartH).append("g").attr("class","legendArea").attr("transform", "translate(" + 0 + "," + 0 + ")");            
            var bulletArea = vis.append("svg").attr("width", chartW + MARGIN.left + MARGIN.right).attr("height", chartH).append("g").attr("class","bulletArea").attr("transform", "translate(" + legendPad + "," + 0 + ")");
            ...
            var filter = defs.append("filter").attr("id", "dropshadow2")
    ...
    */
        function addSymbol(_selection, height) {

            _selection.append('circle').attr('cx',0.5*height)
                                        .attr('cy',0.5*height)
                                        .attr('r',0.20*height)
                                        .attr("fill","#434C4C")
                                        .on("mouseover",function(){
    ...
    */
        function ColorPicker(colorScale) {
            ...
            self.picked = function (fill) {
                //self.rect.setAttribute("fill",fill);
