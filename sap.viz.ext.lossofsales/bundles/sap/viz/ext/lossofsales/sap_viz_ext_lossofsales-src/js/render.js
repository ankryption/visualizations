/*<<dependency*/
define("sap_viz_ext_lossofsales-src/js/render", ["sap_viz_ext_lossofsales-src/js/utils/util"], function(util){
/*dependency>>*/ 
...
*/
    ...
    */
        function customColumnChart(config) {
                ...
                var rectGroup = chart_container.selectAll("g")
                    ...
                    rectGroup.append("rect")
                                        ...
                                        .on("mouseover",function(d,i){
                                            ...
                                            rectGroup.append("line").attr("class", "tooltipLine")
                                                                                    .attr("style","dashed")
                                                                                    .attr("x1",xScale(d.x))
                                                                                    .attr("y1",0)
                                                                                    .attr("x2",xScale(d.x))
                    ...
                    rectGroup.append("text")
                                        ...
                                        .attr("class",function(d){
                                            if(d.text === 0 && d.value === "sale")
                                                return "zeroSaleValue";
                                            else if(d.text === 0 && d.value === "stock")
                                                return "zeroStockValue";
                                            else 
                                                return "centerValue";
                                            })
                                        .text(function(d,i){
                                        ...
                                        .on("mouseover",function(d,i){
                                            ...
                                            var rect_object = d3.select(this.parentNode).select(".objects");
                                        ...
                                        .on("mouseout",function(d,i){
                                                        var object = d3.select(this.parentNode).select(".objects");
            ...
            this.toolTipPopulate = function(d){
                    var tt_data="<div class='tt-item tt-combo'>"+(d.item)+"</div>";
                    if(d.value==="sale"){
                        var firstLabel="<div class='firstLabel tt-item tt-bag-val ' > Loss of Sales ";
                        firstLabel=firstLabel+"<span class='tt-item-price'> "+"$"+" "+d.width+"</span></div>";

                        var secondLabel="<div class='secondLabel tt-item tt-bag-val'> Actual Sales";
                        secondLabel=secondLabel+"<span class='tt-item-price'>"+"$"+" "+d.x+"</span></div>";

                        var thirdLabel="<div class='thirdLabel tt-item tt-bag-val'> Possible Sales";
                        thirdLabel=thirdLabel+"<span class='tt-item-price'>"+"$"+" "+(parseInt(d.x)+parseInt(d.width))+"</span></div>";  

                        tt_data = tt_data+firstLabel+secondLabel+thirdLabel;

                    }   
                    else if(d.value==="stock"){
                        tt_data=tt_data+"<div class='firstLabel tt-item tt-bag-val' > Shortage of Quantity";
                        tt_data=tt_data+"<span class='tt-item-price'>"+" "+(parseFloat(maxOfStock(data)) - parseFloat(d.width))+"</span></div>";

                        tt_data=tt_data+"<div class='secondLabel tt-item tt-bag-val'> Actual Quantity";
                        tt_data=tt_data+"<span class='tt-item-price'>"+" "+(parseFloat(d.x) - (parseFloat(maxOfStock(data)) - parseFloat(d.width)))+"</span></div>";

                        tt_data=tt_data+"<div class='thirdLabel tt-item tt-bag-val'> Forecasted Quantity";
                        tt_data=tt_data+"<span class='tt-item-price'>"+" "+parseFloat(d.x)+"</span></div>";
                    }

                if(clickCounter === 0){
                    if(d.value==="sale"){
                        tt_data=tt_data.replace("selected","thirdLabel");
                        tt_data=tt_data.replace("firstLabel","selected");
                    }
                    else if(d.value==="stock"){
                        tt_data=tt_data.replace("selected2","thirdLabel");
                        tt_data=tt_data.replace("firstLabel","selected2");
                    }
                } 
                else if(clickCounter === 1){
                    if(d.value==="sale"){
                        tt_data=tt_data.replace("selected","firstLabel");
                        tt_data=tt_data.replace("secondLabel","selected");
                    }
                    else if(d.value==="stock"){
                        tt_data=tt_data.replace("selected2","firstLabel");
                        tt_data=tt_data.replace("secondLabel","selected2");
                    }
                }
                else if(clickCounter === 2){
                    if(d.value==="sale"){
                        tt_data=tt_data.replace("selected","secondLabel");
                        tt_data=tt_data.replace("thirdLabel","selected");
                    }
                    else if(d.value==="stock"){
                        tt_data=tt_data.replace("selected2","firstLabel");
                        tt_data=tt_data.replace("thirdLabel","selected2");
                    }
                }
            ...
            this.outMouse = function(object){
                ...
                d3.selectAll("#t-tip").style("display","none");
            ...
            this.onClick = function(d){   
                ...
                d3.select('.stockDiv').selectAll("text").text(function(d,i){
                                                             if (clickCounter === 0)
                                                                 return textFormatter(d.text0);
                                                             else if (clickCounter === 1)
                                                                 return textFormatter(d.text1);
                ...
                d3.select('.salesDiv').selectAll("text").text(function(d,i){
                                                             if (clickCounter === 0)
                                                                 return textFormatter(d.text0);
                                                             else if (clickCounter === 1)
                                                                 return textFormatter(d.text1);
                ...
                if(clickCounter === 0){
                    if(d.value==="sale"){
                        tooltipData=tooltipData.replace("selected","thirdLabel");
                        tooltipData=tooltipData.replace("firstLabel","selected");
                    }
                    else if(d.value==="stock"){
                        tooltipData=tooltipData.replace("selected2","thirdLabel");
                        tooltipData=tooltipData.replace("firstLabel","selected2");
                    }
                } 
                else if(clickCounter === 1){
                    if(d.value==="sale"){
                        tooltipData=tooltipData.replace("selected","firstLabel");
                        tooltipData=tooltipData.replace("secondLabel","selected");
                    }
                    else if(d.value==="stock"){
                        tooltipData=tooltipData.replace("selected2","firstLabel");
                        tooltipData=tooltipData.replace("secondLabel","selected2");
                    }
                }
                else if(clickCounter === 2){
                    if(d.value==="sale"){
                        tooltipData=tooltipData.replace("selected","secondLabel");
                        tooltipData=tooltipData.replace("thirdLabel","selected");
                    }
                    else if(d.value==="stock"){
                        tooltipData=tooltipData.replace("selected2","secondLabel");
                        tooltipData=tooltipData.replace("thirdLabel","selected2");
                    }
                }
    ...
    */
        function update() {
            ...
            var legendDiv = container.append("svg").attr('width', width).attr('height', MARGIN.top).attr("class","legendDiv");
            var margin = container.append("svg").attr('width', MARGIN.left).attr('height', height - MARGIN.top).attr("class","marginDiv");
            var stockVis = container.append('svg').attr('width', chartW).attr('height', height - MARGIN.top).attr("class","stockDiv");
            var centerVis = container.append('svg').attr('width', centerWidth).attr('height', height - MARGIN.top).attr("class","centerDiv");
            var salesVis = container.append('svg').attr('width', chartW).attr('height', height - MARGIN.top).attr("class","salesDiv");
            ...
            var axis_stock = legendDiv.append("g").attr("class","xAxis").call(customChartStock.xAxis).attr("transform", "translate(" + MARGIN.left + "," + MARGIN.top + ")");
            var axis_sales = legendDiv.append("g").attr("class","xAxis").call(customChartSales.xAxis).attr("transform", "translate(" + (MARGIN.left + centerWidth + chartW) + "," + MARGIN.top + ")");
            ...
            var centerRect = centerVis.append("rect").attr("class","pane")
            ...
            var zoom = d3.behavior.zoom();
                zoom = zoom.y(yscale);
                zoom = zoom.scaleExtent([1, stockData.length/2]).on("zoom", redraw);
            ...
            function redraw() {
                ...
                var t = d3.event.translate;
                    ...
                    stockVis.selectAll(".stockObjects").remove();
                    salesVis.selectAll(".salesObjects").remove();
    ...
    */
        function tooltipDefine () {
            ...
            function tooltip() {
                ...
                var  tt=d3.select("body").append("div").attr("id","t-tip").style("display",disp);
                var tooltip=tt.append("div").attr("class","tt-inner").style("text-align","left");
                ...
                tooltip.visibility=function(value){
                    ...
                    d3.selectAll("#t-tip").style("display",value);
                }

                tooltip.orientation=function(value)        {
                    //if (!arguments.length) return html;
                    d3.selectAll("#t-tip #triangle").attr("class",value);
                }

                tooltip.move=function(value) {

                    d3.selectAll("#t-tip").style("top",value[0]).style("left",value[1]);
                };

                tooltip.fillData=function(value) {

                    d3.selectAll("#t-tip #tt-body").html(value);
                };

                tooltip.getData=function() {
                    return(d3.selectAll("#t-tip #tt-body").html());
                };
                return tooltip;
            }
